...
async function loadPartition() {
  ...
  for (const s of symbols) {
    ...
    // Tri-state, deliberately: a market-watch row that's PRESENT but has no
    // volume figure (its table cell is blank — a real no-trade day) means
    // "known: zero trades," so it's set to `null` and treated as illiquid
    // below. A symbol with NO row at all (missing from the table, or the
    // whole market-watch fetch failed) means "unknown" and stays
    // `undefined` — never excluded on missing data, only on a confirmed
    // thin/no-trade day.
    s.volume = info ? (info.volume ?? null) : undefined;
