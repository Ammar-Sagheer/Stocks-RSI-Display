...
async function buildStockRecord(meta) {
  ...
  // current price so the still-forming daily/weekly/monthly candle matches
  ...
  adjusted[adjusted.length - 1] = { ...latest, close: price };
  ...
  const record = {
    ...
    history: adjusted.slice(-120).map((p, idx, arr) => ({
      date: p.date,
      close: p.close,
      rsi: dailyRsiSeries[adjusted.length - arr.length + idx] ?? null,
    })),
    ...
    rsiWeekly: latestRSI(resampleCloses(adjusted, "week")),
    rsiMonthly: latestRSI(resampleCloses(adjusted, "month")),
  };
