...
async function buildStockRecord(meta) {
  ...
  const dailyRsiSeries = calculateRSISeries(closes);
  ...
  const record = {
    ...
    sector: meta.sector,
    isETF: meta.isETF,
    // Live CURRENT price from market-watch; fall back to the latest EOD close
    // only when market-watch didn't list a price for this symbol.
    price: meta.currentPrice ?? latest.close,
    asOf: latest.date,
    history: series.slice(-120).map((p, idx, arr) => ({
      date: p.date,
      close: p.close,
      rsi: dailyRsiSeries[series.length - arr.length + idx] ?? null,
    })),
    // RSI(14) on daily / weekly / monthly candles — matches TradingView's
    // per-timeframe RSI. Weekly/monthly resample the daily closes into the
    // last close of each week/month first.
    rsiDaily: lastRSIValue(dailyRsiSeries),
    rsiWeekly: latestRSI(resampleCloses(series, "week")),
    rsiMonthly: latestRSI(resampleCloses(series, "month")),
  };
