...
async function buildStockRecord(meta) {
  ...
  const weeklyCloses = resampleCloses(adjusted, "week");
  const monthlyCloses = resampleCloses(adjusted, "month");

  // RSI at every selectable look-back period, each on daily / weekly /
  // monthly candles — matches TradingView's per-timeframe RSI at that
  // period. All periods share the same closes, so computing the full grid
  // here is nearly free and lets the frontend switch periods instantly
  // without refetching.
  const rsi = {};
  const dailySeriesByPeriod = {};
  for (const { period } of RSI_PERIODS) {
    const dailySeries = calculateRSISeries(closes, period);
    dailySeriesByPeriod[period] = dailySeries;
    rsi[period] = {
      daily: lastRSIValue(dailySeries),
      weekly: lastRSIValue(calculateRSISeries(weeklyCloses, period)),
      monthly: lastRSIValue(calculateRSISeries(monthlyCloses, period)),
    };
  }
  ...
  const record = {
    ...
    asOf: latest.date,
    history: adjusted.slice(-120).map((p, idx, arr) => {
      const i = adjusted.length - arr.length + idx;
      const rsiAt = {};
      for (const { period } of RSI_PERIODS) {
        rsiAt[period] = dailySeriesByPeriod[period][i] ?? null;
      }
      return { date: p.date, close: p.close, rsi: rsiAt };
    }),
    rsi,
  };
