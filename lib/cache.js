...
async function buildStockRecord(meta) {
  ...
  // Latest RSI at every look-back period × chart interval, computed on
  // candles resampled from the full daily history — matches TradingView's
  // RSI at that period and chart interval. The whole grid shares the same
  ...
  // shipped here — the /api/history endpoint computes it on demand from
  // `seriesBySymbol`, which keeps the main payload small.
  const rsi = {};
  ...
  // Swing-entry signal on daily candles: beaten down on RSI(14) AND
  // short-term stretched on RSI(2) right now. See BUY_SIGNAL in lib/rsi.js.
  const r14 = rsi[14]["1D"];
  const r2 = rsi[2]["1D"];
  const buySignal =
