...
// PSX resets connections (UND_ERR_SOCKET) when hit with too many parallel
// requests — that's rate-limiting, not a hard IP block. Keeping concurrency
// low means far fewer requests fail on the first pass, which is faster
// overall than a high concurrency that thrashes on retries.
const CONCURRENCY = 4;
const RETRY_CONCURRENCY = 2;
const RETRY_PASS_DELAY_MS = 10000; // let PSX's rate limiter cool down before retrying
...
// blocking until every symbol is done. This lives in the server process's
// memory, so it needs a long-lived server (Render, a VPS) to persist across
// requests — not a per-request serverless function.
const state = {
  bySymbol: new Map(),
  totalCount: 0,
  loadedCount: 0,
...
async function refresh() {
  ...
  try {
    ...
    try {
      ...
      console.error(`[cache] fetchSymbols failed after retries: ${err.name}: ${err.message}`);
      throw err;
    }
